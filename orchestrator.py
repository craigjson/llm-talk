...
class Orchestrator:
    def __init__(
        ...
        self.messages = [
            {
                "role": "system",
                "content": "You are a storyteller who loves to make up fantastic, fun, and educational stories for children between the ages of 5 and 10 years old. Your stories are full of friendly, magical creatures. Your stories are never scary. Each sentence of your story will become a page in a storybook. Stop after 3-4 sentences and give the child a choice to make that will influence the next part of the story. Once the child responds, start by saying something nice about the choice they made, then include [start] in your response. Include [break] after each sentence of the story. Include [prompt] between the story and the prompt.",
            }
        ]
        self.intro_messages = [
            {
                "role": "system",
                "content": "You are a storyteller who loves to make up fantastic, fun, and educational stories for children between the ages of 5 and 10 years old. Your stories are full of friendly, magical creatures. Your stories are never scary. Begin by asking what a child wants you to tell a story about.",
            }
        ]
    ...
    def handle_user_speech(self, user_speech):
        ...
        if not self.llm_response_thread or not self.llm_response_thread.is_alive():
            ...
            self.llm_response_thread = Thread(
                target=self.request_llm_response, args=(user_speech,)
            )
                        ...
                        if re.findall(r".*\[[sS]tart\].*", out):
                            ...
                            if len(out) > 2:
                                ...
                                self.enqueue(
                                    StoryPageAsyncScene,
                                    sentence=out,
                                    image=self.image_this_time,
                                    story_sentences=self.story_sentences.copy(),
                                )
    ...
    def request_image_description(self, story_sentences):
        ...
        self.logger.info(
            f"==== time to run llm for image generation {time.time() - start}"
        )
        return image_prompt
